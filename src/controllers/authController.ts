...
export class AuthController {
  static async login(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      // Supporte bcrypt ET texte clair
      let isPasswordValid = false;
      if (passwordField.startsWith('$2b$') || passwordField.startsWith('$2a$')) {
      ...
      const displayName: string = user.name || `${user.first_name || ''} ${user.last_name || ''}`.trim();

      console.log('✅ Login réussi pour:', user.email, '| rôle:', role, '| id:', user.id);
