...
export class ExamController {
  ...
  static async getExamsHistory(_req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
  ...
  static async create(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      const { course_id, title, description, start_time, end_time, duration_minutes } = req.body;
      if (!course_id || !title || !start_time || !end_time || !duration_minutes) {
        res.status(400).json({ message: 'Missing required fields' });
        return;
      }

      const exam = await ExamService.createExam({ course_id, title, description, start_time, end_time, duration_minutes });
  ...
  static async update(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const exam = await ExamService.updateExam(id, { course_id, title, description, start_time, end_time, duration_minutes });
      res.status(200).json(exam);
    } catch (error) {
      next(error);
    }
  ...
  // 🟢 NOUVELLE MÉTHODE : Contrôleur pour la soumission
  static async submitExam(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      if (!Array.isArray(answers)) {
        res.status(400).json({ message: 'Invalid format: answers must be an array' });
        return;
      }
