...
export class QuestionRepository {
  static async findByExamId(examId: number, includeCorrect: boolean = true) {
    ...
    for (const q of questions) {
      let choicesQuery =
        "SELECT id, choice_text FROM choices WHERE question_id = $1 ORDER BY id ASC";
      if (includeCorrect) {
        choicesQuery =
          "SELECT id, choice_text, is_correct FROM choices WHERE question_id = $1 ORDER BY id ASC";
      }
  ...
  static async createQuestion(
    ...
    const query =
      "INSERT INTO questions (exam_id, question_text, points) VALUES ($1, $2, $3) RETURNING *";
    const result = await pool.query(query, [examId, questionText, points]);
    return result.rows[0];
  }

  static async createChoice(
    questionId: number,
    choiceText: string,
    isCorrect: boolean,
  ) {
    const query =
      "INSERT INTO choices (question_id, choice_text, is_correct) VALUES ($1, $2, $3) RETURNING *";
