...
export class QuestionService {
  ...
  static async addQuestionToExam(
    ...
    if (hasAttempts) {
      const error: any = new Error(
        "Cannot modify questions because exam has attempts (RG-08)",
      );
      error.status = 409;
      throw error;
    }
    ...
    if (!data.choices || data.choices.length < 2 || data.choices.length > 6) {
      const error: any = new Error(
        "A question must have between 2 and 6 choices (RG-04)",
      );
      error.status = 400;
      throw error;
    }
    ...
    if (correctCount !== 1) {
      const error: any = new Error(
        "A question must have exactly one correct choice (RG-04)",
      );
      error.status = 400;
      throw error;
    }
    ...
    for (const choice of data.choices) {
      const c = await QuestionRepository.createChoice(
        question.id,
        choice.choice_text,
        choice.is_correct,
      );
  ...
  static async deleteQuestion(questionId: number) {
    ...
    if (hasAttempts) {
      const error: any = new Error(
        "Cannot delete question because exam has attempts (RG-08)",
      );
      error.status = 409;
      throw error;
    }
